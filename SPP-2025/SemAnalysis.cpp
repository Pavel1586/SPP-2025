...
namespace Semantic
{
	bool Semantic::semanticsCheck(Lexer::LEX& tables, Log::LOG& log)
	{
		...
		for (int i = 0; i < tables.lextable.size; i++)
		{
			...
			case LEX_EQUAL:
			{
				if (i > 0 && tables.lextable.table[i - 1].idxTI != NULLIDX_TI)
				{
					...
					for (int k = i + 1; tables.lextable.table[k].lexema != LEX_SEPARATOR; k++)
					{
						...
						// Проверка на операции сравнения в выражении присваивания - результат сравнения (0 или 1) можно присваивать переменной типа integer
						if (lefttype == IT::IDDATATYPE::INT)
						{
							char l = tables.lextable.table[k].lexema;
							if (l == LEX_MORE || l == LEX_LESS || l == LEX_EQUALS || l == LEX_NOTEQUALS || l == LEX_MOREEQUALS || l == LEX_LESSEQUALS)
							{
								// Результат сравнения - integer (0 или 1), это допустимо для присваивания
								ignore = true; // пропускать проверку типов для операндов сравнения внутри скобок
							}
						}
					}
				}
			...
			case LEX_MORE:	case LEX_LESS: case LEX_NOTEQUALS:	case LEX_MOREEQUALS:	case LEX_LESSEQUALS:
			{
				bool flag = true;
			...
			case LEX_BREAK:
			{
				// Switch-конструкция не требует дополнительной семантической проверки
				break;
			}
			}
		}
