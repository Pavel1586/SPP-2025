...
namespace Polish
{
    int getPriority(LT::Entry& e)
    {
        ...
        case LEX_MORE: case LEX_LESS: case LEX_EQUALS: case LEX_NOTEQUALS: case LEX_MOREEQUALS: case LEX_LESSEQUALS: return 0;
        default: return -1;
        }
    }
    ...
    bool __cdecl setPolishNotation(IT::IdTable& idtable, Log::LOG& log, int lextable_pos, ltvec& v)
    {
        ...
        for (unsigned i = 0; i < v.size(); i++)
        {
            ...
            // Проверяем только необработанные лексемы
            if (v[i].lexema != LEX_LEFTHESIS && v[i].lexema != LEX_RIGHTTHESIS && v[i].lexema != LEX_PLUS && 
                v[i].lexema != LEX_MINUS && v[i].lexema != LEX_STAR && v[i].lexema != LEX_DIRSLASH && 
                v[i].lexema != LEX_PERSENT && v[i].lexema != LEX_ID && v[i].lexema != LEX_LITERAL && 
                v[i].lexema != LEX_LEFT && v[i].lexema != LEX_RIGHT && v[i].lexema != LEX_MORE && 
                v[i].lexema != LEX_LESS && v[i].lexema != LEX_EQUALS && v[i].lexema != LEX_NOTEQUALS && 
                v[i].lexema != LEX_MOREEQUALS && v[i].lexema != LEX_LESSEQUALS)
            {
