		...
		if (lenta_position < lenta_size)
		{
			if (ISNS(st.top())) //    
			{
				...
				if ((nrule = grebach.getRule(st.top(), rule)) >= 0)//,   
				{
					...
					{
						// Проверяем наличие пустого правила (epsilon) - цепочки с size == 0
						bool found_epsilon = false;
						for (short k = nrulechain + 1; k < rule.size; k++)
						{
							if (rule.chains[k].size == 0)
							{
								chain = rule.chains[k];
								nrulechain = k;
								found_epsilon = true;
								MFST_TRACE1(log)//
									savestate(log);//  
								st.pop();//  .
								push_chain(chain); // (пустая цепочка ничего не добавит)
								rc = NS_OK; //... 
								MFST_TRACE2(log);//
								break;
							}
